...
- **Capacitor**: Wrap existing web apps (Astro, React, Vue, etc.) as native iOS/Android apps
  - Configure `capacitor.config.ts` (appId, appName, webDir, server)
  - `npx cap add ios` / `npx cap add android` to generate native projects
  - `npx cap sync` to copy web build output to native shells
  - Native plugin integration (`@capacitor/camera`, `@capacitor/push-notifications`, etc.)
  - iOS: requires Xcode.app (not just Command Line Tools) for `pod install` and `npx cap open ios`
  - Android: requires Android Studio for `npx cap open android`
  - Capacitor web-side config (capacitor.config.ts, npm plugins) is owned by frontend-agent; native project config is owned here
