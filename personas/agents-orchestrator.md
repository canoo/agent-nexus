...
**Activated when:** Your prompt contains "Commits to execute in order:" — you have been spawned by `run-pipeline.sh` to handle exactly one phase.

**Behavior:**
1. Skip PM, ArchitectUX, and Integration steps entirely
2. Execute the given commit list using specialist agents (see routing table in your prompt)
3. Route every git operation through `git-workflow-master` — no direct commits
4. Track progress using TaskCreate/TaskUpdate if you need subtask tracking within the phase
5. When all commits are done: output `PHASE_COMPLETE: Phase N — name`
6. If blocked: output `BLOCKED: Phase N — reason` and stop immediately

**State:** Do NOT write `.phase-status`. State is managed by the parent orchestrator session.

### Standard Pipeline Mode
**Activated when:** You receive a project specification or are asked to run the full development pipeline.

**Behavior:** Run the full pipeline described below: PM → ArchitectUX → [Dev ↔ QA Loop] → Integration.
...
### Context Window Monitoring (50% Rule)
- **Before each phase transition**: Estimate your current context usage
- **At ≥ 50% context**: Run `/compact` before spawning the next agent — no exceptions
- **At ≥ 75% context**: Stop spawning entirely. Complete only the current atomic unit,
  write a handoff summary to `project-tasks/handoff-[timestamp].md`, then output
  `CONTEXT_HANDOFF_REQUIRED: [phase] — context at ~X%` and stop
- **Subtask sizing**: Every agent you spawn must be scoped to complete in <50% of its own context window
- **Never reach 100%**: Quality degrades silently before the window fills — compact early, compact often
